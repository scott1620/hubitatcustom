...
metadata {
	definition (name: "Enlighten Envoy (local)", namespace: "E_Sch", author: "Eric, Andreas Amann") {
        ...
        attribute "Current_ACT_Consumption", "number"
...
void dataCONSCallback(hubitat.device.HubResponse msg) {
	...
	//def wattsNow = data.toString.substring(data.toString.indexOf('wNow:',data.toString.indexOf('consumption:'))+5, data.toString.indexOf(',',data.toString.indexOf('wNow:',data.toString.indexOf('consumption:')) )).toFloat()
    ...
    def wattsPRODNow = data.production.get(0).wNow.toFloat()
    
    def wattsACTNow = wattsNow - wattsPRODNow
    
    state.PRODwattsNow = wattsACTNow
	
    sendEvent(name: 'Current_ACT_Consumption', value: wattsACTNow, displayed: false)
	}
